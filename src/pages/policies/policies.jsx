...
import { getPoliciesPageData } from '../../lib/queries';
...
const colOne = [
	{
		...
		bg: (
			<div className=' h-full z-0 absolute w-full top-0 left-0'>
				<div className='absolute top-[-55.9px] left-[-28px]'>{curveOneTop}</div>
	...
	},
	{
		...
		text: (
			<>
				Get ready for a blast because all our camp activities are crafted to be
				age-appropriate, safe, and super FUN! Our golden rule? Pick an activity,
				give it a shot, and stick with it until the next period. We’re all about
				getting active—think playing in the dirt, splatter paint, and running
				through hoses on a hot day! Please note: campers won’t be sidelined from
				activities for aesthetic reasons like clothing or hair—parents we ask
				you to prepare them for all the fun, let’s keep it fun and free! Just a
				heads-up: it’s on parents to inform us about any medical reasons that
				might prevent a camper from participating.{' '}
			</>
		),
		bg: (
			<div className=' h-full z-0 absolute w-full top-0 left-0'>
	...
	},
	{
		...
		text: (
			<>
				...
				are allowed during camp hours. Campers can stash them in their bags, but
				direct contact with your kiddo is a no-go; it just makes those rules go
				haywire! If you need to reach out, give us a ring at 404.955.3184. Let’s
...

	{
		...
		text: (
			<>
				...
				You’ll have that number to reach us in case of urgent messages. Text if
				it’s something quick and easy you need to get to us. Otherwise give us a
				call. Transparency and communication are key to our parent partnership.
			</>
		),
		bg: (
			<div className=' h-full z-0 absolute w-full top-0 left-0'>
...
const colTwo = [
	{
		...
		text: (
			<>
				...
				name. We ask that breakfast be fed prior to camp. On Wednesdays we wear
				our Kiddos Shirts!
			</>
		),
		bg: (
	...
	},
	{
		...
		bg: (
			<div className=' h-full z-0 absolute w-full top-0 left-0'>
				<div className='absolute top-[-55.9px] left-[-38px]'>{curveOneTop}</div>
			</div>
		),
	},
	{
...
function Policies() {
	...
	useEffect(() => {
		getPoliciesPageData().then(setData);
	}, []);
	// useEffect(() => {
	// 	console.log(data);
	// }, [data]);
	if (!data) return <div>loading Page</div>;
	const { pageDescription, policies, pageTitle } = data;

	// 1. Array Merging Logic (Put it here!)
	const mobilePolicies = [];
	const maxLen = Math.max(colOne.length, colTwo.length);
	const FirstCol = policies.filter((_, i) => i % 2 === 0);
	const LastColIndex = FirstCol.length - 1;
	const SecCol = policies.filter((_, i) => i % 2 === 1);
	SecCol.push(FirstCol[LastColIndex]);
				...
				<div className='max-w-[1243.406px]   w-full relative lg:px-[73px] px-[60px] pb-[50px]  lg:pb-[200px] pt-[50px] lg:pt-[137px] mx-auto'>
					<div className=' lg:block hidden space-y-[20px] '>
						...
						<p className="max-w-[391px] justify-center text-[#004aad] text-[25px] font-medium font-['Montserrat'] leading-8">
							{renderPreTitle(pageDescription) || (
								<>
									To ensure a smooth and enjoyable camp experience for everyone,
									please take note of our key policies and some tips for
									parents. <br />
									<br />
									Our full camp handbook will be provided after registration,
					...
					<div className='space-y-[30px] lg:hidden'>
						<p className="max-w-[309px]  justify-start text-[#004aad] text-[50px] font-bold font-['League_Spartan'] leading-[44.50px]">
							{renderPreTitle(pageTitle) || <>Tips for Parents & Guardians</>}{' '}
							<br />
						</p>
				...
				<div className='max-w-[1243.406px]   w-full relative lg:px-0 px-[50px] pb-[100px] lg:pb-[200px] pt-[50px] lg:pt-[240px] mx-auto'>
					<div className='flex lg:flex-row items-center lg:items-start flex-col justify-center space-y-[51px] lg:space-y-0 lg:space-x-[141px]'>
						<div className='w-[331px] '>
							<div className='lg:block hidden'>
								{(data ? FirstCol.slice(0, -1) : colOne).map((col, i) => (
									<div
										style={{
											marginBottom: colOne[i]?.specialStyle
												? colOne[i].specialStyle
												: '51px',
										}}
							...
							<div className='lg:hidden'>
								{policies.map((col, i) => (
									<div
										style={{
											marginBottom: mobilePolicies[i]?.specialStyle
												? mobilePolicies[i].specialStyle
												: '51px',
										}}
										className={`w-full max-w-[360px] lg:w-[360px]  relative space-y-[22px] pb-[60px] py-[26px] lg:py-[31.56px] pl-[32px] lg:pl-[50px] px-[26px]  ${
											mobilePolicies[i]?.specialStyle
						...
						<div className='w-[331px] lg:block hidden space-y-[51px]'>
							{(data ? SecCol : colTwo).map((col, i) => (
								<div
									style={{
										marginBottom: colTwo[i]?.specialStyle
											? colTwo[i].specialStyle
											: '51px',
									}}
									className={`w-full max-w-[360px] lg:w-[360px]  relative space-y-[22px] pb-[60px] py-[26px] lg:py-[31.56px] pl-[32px] lg:pl-[50px] px-[26px]  ${
										colTwo[i]?.specialStyle
